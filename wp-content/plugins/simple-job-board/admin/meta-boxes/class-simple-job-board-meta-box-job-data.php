...
class Simple_Job_Board_Meta_Box_Job_Data {
     ...
     * 
     * @since 2.1.0
     * @since 2.9.6	Removed $html variable and added esc_attr() with output variables.
     * 
    ...
    public static function text($id, $label, $desc = '') {
        ...
        <p class="metabox-field">
            <label for="simple_job_board<?php echo esc_attr($id); ?>"><?php echo esc_attr($label); ?></label>
            <input type="text" id="simple_job_board<?php echo esc_attr($id); ?>" name="simple_job_board<?php echo esc_attr($id); ?>" value="<?php echo esc_attr(get_post_meta($post->ID, 'simple_job_board' . $id, TRUE)); ?>" />
            <?php
            if ($desc) {
                echo '<span class="tips">' . esc_attr($desc) . '</span>';
            }
            echo '</p>';
        }
         ...
         * 
         * @since 2.1.0
         * @since 2.9.6  Added esc_attr() with output variables.
         * 
         * @param   string   $id     Field id
         * @param   string   $label  Field lable
         * @param   string   $desc   Field description
        ...
        <p class="metabox-field">
            <label for="simple_job_board<?php echo esc_attr($id); ?>"><?php echo esc_attr($label); ?></label>            
            <span class="file_url">
                <input type="text" name="simple_job_board<?php echo esc_attr($id); ?>" id="simple_job_board<?php echo esc_attr($id); ?>" class="upload_field" placeholder="URL to the company logo" value="<?php echo esc_url(get_post_meta($post->ID, 'simple_job_board' . $id, TRUE)); ?>" />
