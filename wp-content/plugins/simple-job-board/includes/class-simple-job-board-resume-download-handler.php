...
class Simple_Job_Board_Resume_Download_Handler {
    ...
    public function download_resume() {
        ...
        if (isset($_GET['resume_id'])) {
            ...
            $resume = get_post_meta(intval(sanitize_text_field($_GET['resume_id'])), 'resume_path', TRUE);
            ...
            $files = get_post_meta(intval(sanitize_text_field($_GET['post'])), 'attachments_meta', TRUE);
            $file_path = SJB_UPLOAD_DIR . substr(strrchr($files['base_dir'], "/"), 1) . '/' . sanitize_file_name($_GET['sjb_file']);
        }

        $file_extension = strtolower(substr(strrchr($file_path, "."), 1));

        // Allowed file extension
        $allowed_file_exts = $this->get_sjb_allowed_extension();

        if ( !empty($allowed_file_exts) && in_array( $file_extension, $allowed_file_exts ) ) {
            ...
            wp_die(__('You are not allowed to download this file. (If you believe you are authorized user then deactivate and reactivate Simple Job Board Plugin.)', 'simple-job-board'), '', array('response' => 404));
        }
     ...
     * @access  private
     * @since   2.4.4
     * @since   2.9.5   Added page redirect link to the 
     */
    private function check_user_login() {
        ...
        if (!is_user_logged_in()) {
            ...
            wp_die(__('You must be logged in to download files.', 'simple-job-board') . ' <a href="' . esc_url($redirect) . '" >' . __('Login', 'simple-job-board') . '</a>', 403);
    ...
    private function readfile_chunked( $file ) {
        ...
        while (!@feof($handle)) {
           echo @fread($handle, $chunksize); // phpcs.ignore - PHP File Read Function
     ...
     */
    private function get_sjb_allowed_extension() {
        $allowed_file_exts = array('pdf', 'doc', 'docx', 'odt', 'rtf', 'txt');
        $save_file_exts = get_option('job_board_allowed_extensions');
        $allowed_file_exts = array_unique(array_merge($allowed_file_exts, $save_file_exts));
        return $allowed_file_exts;
    }
